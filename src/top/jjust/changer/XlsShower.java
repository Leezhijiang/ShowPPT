...
public class XlsShower {
	/**
     * ����
     * 
     * @param args
     */
    public static String convert2Html(InputStream is,String roomID) {
        ...
        try {
            ...
            if (wb instanceof XSSFWorkbook) {
                ...
                htmlExcel = XlsShower.getExcelInfo(xWb, true);
            } else if (wb instanceof HSSFWorkbook) {
                HSSFWorkbook hWb = (HSSFWorkbook) wb;
                htmlExcel = XlsShower.getExcelInfo(hWb, true);
            }
            htmlExcel = htmlExcel.replace("<table", "<table style=\"border:1;\" border='1' cellspacing=\"0\" cellpadding=\"0\"");
            htmlExcel = HtmlUtils.getHtml(htmlExcel);
            FileUtils.writeFile(htmlExcel, StaticVariable.PPTPicRootPath+roomID+"/"+"my.html");
            return htmlExcel;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public String readExcelToHtml(InputStream is, boolean isWithStyle) {
        ...
        try {
            ...
            if (wb instanceof XSSFWorkbook) {
                XSSFWorkbook xWb = (XSSFWorkbook) wb;
                htmlExcel = XlsShower.getExcelInfo(xWb, isWithStyle);
            } else if (wb instanceof HSSFWorkbook) {
                HSSFWorkbook hWb = (HSSFWorkbook) wb;
                htmlExcel = XlsShower.getExcelInfo(hWb, isWithStyle);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
